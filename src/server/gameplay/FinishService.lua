...
-- Track how many times each player has reached the finish
local winCounts = {} -- [userId] = integer
...
local function showWinGui(player, winCount, totalRuns)
    if not player or not player:FindFirstChild("PlayerGui") then return end
    ...
    -- Build a message like "🎉 CONGRATS! This is your 3rd win of 10 runs! 🎉"
    local mainMessage
    local winOrdinal = (winCount and winCount > 0) and ordinal(winCount) or nil

    if winOrdinal and totalRuns and totalRuns > 0 then
        mainMessage = string.format(
            "🎉 CONGRATS! This is your %s win of %d runs! 🎉",
            winOrdinal,
            totalRuns
        )
    elseif winOrdinal then
        -- Fallback if we don't know totalRuns yet
        mainMessage = string.format(
            "🎉 CONGRATS! This is your %s win! 🎉",
            winOrdinal
        )
    else
        mainMessage = "🎉 CONGRATS! You reached the treasure! 🎉"
    end
...
local function onPlayerFinished(boat)
    ...
    winCounts[userId] = (winCounts[userId] or 0) + 1

    -- Expose it as a Player attribute so UI / AI / env can inspect it
    player:SetAttribute("WinCount", winCounts[userId])

    print(string.format(
        "[FinishService] %s has reached the finish line! Total wins: %d",
        player.Name,
        winCounts[userId]
    ))

    -- Treat this as an episode end: freeze current boat
    -- (If your RL loop respawns/teleports boats, that logic can run after this.)
    freezeBoat(boat)

    -- Celebration every time
    createConfettiAbove(boat)
    showWinGui(player, winCounts[userId], totalRuns)

    -- Optional: award points, give currency, etc. here
end
...
function M.stop()
    ...
    -- Reset win tracking when service stops
    winCounts = {}
end
